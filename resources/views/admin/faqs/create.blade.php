    ...
    <div class="card-body">
        ...
        <form action="{{ route('admin.faqs.store') }}" method="POST" id="faqForm">
                ...
                <div class="tab-pane fade show active" id="id-pane" role="tabpanel">
                    <div class="mb-3">
                        ...
                        <input type="text" name="question" class="form-control @error('question') is-invalid @enderror" 
                               value="{{ old('question') }}" required placeholder="Masukkan pertanyaan">
                        @error('question')
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="answer" id="answer-hidden-id" class="d-none" required>{{ old('answer') }}</textarea>
                        @error('answer')
                ...
                <div class="tab-pane fade" id="en-pane" role="tabpanel">
                    <div class="mb-3">
                        ...
                        <input type="text" name="question_en" class="form-control @error('question_en') is-invalid @enderror" 
                               value="{{ old('question_en') }}" placeholder="Enter question">
                        @error('question_en')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="answer_en" id="answer-hidden-en" class="d-none">{{ old('answer_en') }}</textarea>
                        @error('answer_en')
                            <div class="invalid-feedback d-block">{{ $message }}</div>
                        @enderror
                    </div>
            ...
            <div class="d-flex justify-content-between">
                ...
                <button type="submit" class="btn btn-primary" onclick="updateHiddenInputs(); return true;">
...
@if(old('answer'))
    quillId.root.innerHTML = {!! json_encode(old('answer')) !!};
@endif

@if(old('answer_en'))
    quillEn.root.innerHTML = {!! json_encode(old('answer_en')) !!};
@endif
