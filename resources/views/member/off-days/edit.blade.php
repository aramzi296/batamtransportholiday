...
@section('title', 'Edit Hari Off')
@section('page-title', 'Edit Hari Off Kendaraan')
    ...
    <div class="col-md-8 offset-md-2">
        <div class="card">
            <div class="card-header bg-warning text-dark">
                <h5 class="mb-0"><i class="fas fa-calendar-edit"></i> Edit Hari Off Kendaraan</h5>
            </div>
            <div class="card-body">
                @if(session('success'))
                ...
                <form method="POST" action="{{ route('member.off-days.update', $offDay->id) }}">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-3">
                        ...
                        <select name="vehicle_id" id="vehicle_id" class="form-select @error('vehicle_id') is-invalid @enderror" required>
                            ...
                            @foreach($vehicles as $vehicle)
                                <option value="{{ $vehicle->id }}" {{ old('vehicle_id', $offDay->vehicle_id) == $vehicle->id ? 'selected' : '' }}>
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="date" name="start_date" id="start_date" 
                                   ...
                                   value="{{ old('start_date', $offDay->start_date->format('Y-m-d')) }}" 
                                   min="{{ date('Y-m-d') }}" 
                                   required>
                            @error('start_date')
                                <div class="invalid-feedback">{{ $message }}</div>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="date" name="end_date" id="end_date" 
                                   ...
                                   value="{{ old('end_date', $offDay->end_date->format('Y-m-d')) }}" 
                                   min="{{ date('Y-m-d') }}" 
                                   required>
                            @error('end_date')
                                <div class="invalid-feedback">{{ $message }}</div>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="reason" id="reason" rows="4" 
                                  ...
                                  placeholder="Masukkan alasan kendaraan off (opsional)">{{ old('reason', $offDay->reason) }}</textarea>
                        @error('reason')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="d-flex justify-content-between">
                        ...
                        <button type="submit" class="btn btn-warning">
                            <i class="fas fa-save"></i> Update Hari Off
                        </button>
                    </div>
                </form>
            </div>
